...
// [EasyTax SEO] 교민 세금 환급 안내 랜딩 페이지
// Route: /[lang]/community/[slug]
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `[${title}] Korean Tax Refund for Expat Communities | EasyTax Korea`,
    ...
    alternates: {
      canonical: `https://easytax-service.vercel.app/${lang}/community/${slug}`,
      languages: Object.fromEntries(
        Object.keys(LANG_MAP).map(l => [l, `https://easytax-service.vercel.app/${l}/community/${slug}`])
      ),
    },
    openGraph: {
      title: `[${title}] EasyTax 🤝 Korean Tax Refund for Expat Communities`,
      description: `Korean tax refund help near ${title}. ${langInfo.name} supported.`,
      url: `https://easytax-service.vercel.app/${lang}/community/${slug}`,
      siteName: 'EasyTax Korea',
    },
  };
}

export default async function EasyTaxCommunitySEOPage({ params }: Props) {
  const { lang, slug } = await params;
  const langInfo = LANG_MAP[lang] || LANG_MAP['en'];
  const title = slugToTitle(slug);
  const cta = `https://easytax-service.vercel.app/${lang}/welcome?utm_source=google_seo&utm_medium=organic&utm_campaign=community_${slug}`;

  const jsonLd = {
    '@context': 'https://schema.org/',
    '@type': 'GovernmentService',
    name: `EasyTax @ ${title}`,
    description: `Korean Tax Refund for Expat Communities near ${title} in Korea.`,
    url: `https://easytax-service.vercel.app/${lang}/community/${slug}`,
      ...
      <div style={{ fontFamily: "'Inter', sans-serif", background: '#0a0f1e', minHeight: '100vh', color: '#f8fafc' }}>
        ...
        <section style={{ maxWidth: '900px', margin: '0 auto', padding: '60px 24px 40px', textAlign: 'center' }}>
          ...
          <h1 style={{ fontSize: '30px', fontWeight: 900, lineHeight: 1.3, marginBottom: '16px' }}>
            {title} — Korean Tax Refund for Expat Communities
          </h1>
