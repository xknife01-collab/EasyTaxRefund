...
// [EasyTax SEO] 비자별 세무 가이드 페이지
// Route: /[lang]/visa/[slug] & /visa/[slug]
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  return {
    title: `[${title}] Korean Tax Refund & Year-End Settlement Guide | EasyTax Korea`,
    description: `Official tax refund guide for ${title} holders in South Korea. AI guidance in 17 languages. Free simulation.`,
  };
}

export default async function EasyTaxVisaPage({ params, searchParams }: Props) {
  const { slug, lang: pLang } = await params;
  const sp = searchParams ? await searchParams : {};
  const lang = pLang || sp?.lang || 'en';
  const langInfo = LANG_MAP[lang] || LANG_MAP['en'];
  const title = slugToTitle(slug);
  const cta = `/welcome?lang=${lang}&utm_source=google_seo&utm_medium=organic&utm_campaign=visa_${slug}`;
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: '#0a0f1e', minHeight: '100vh', color: '#f8fafc' }}>
      ...
      <main style={{ maxWidth: '800px', margin: '0 auto', padding: '48px 24px' }}>
        <div style={{ background: '#0f1729', border: '1px solid #1e3a5f', borderRadius: '20px', padding: '36px', textAlign: 'center' }}>
          ...
          <h1 style={{ fontSize: '26px', fontWeight: 900, marginBottom: '12px' }}>{title} Korean Tax Refund Guide</h1>
          <p style={{ color: '#64748b', fontSize: '15px', marginBottom: '28px' }}>Maximize your tax refund in Korea. AI-powered year-end settlement & 5-year refund check.</p>
          <a href={cta} style={{ display: 'inline-block', background: '#3b82f6', color: '#fff', fontWeight: 900, padding: '14px 36px', borderRadius: '50px', textDecoration: 'none', fontSize: '16px' }}>Calculate My Tax Refund →</a>
        </div>
      </main>
