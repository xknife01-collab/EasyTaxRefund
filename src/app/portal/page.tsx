        ...
        <DialogContent className="max-w-4xl p-0 overflow-hidden border-none rounded-[2.5rem] shadow-2xl bg-white z-[150]">
          ...
          <div className="bg-white p-12 lg:p-20 overflow-y-auto max-h-[75vh] font-serif text-slate-900">
            ...
            <div ref={printRef} className="space-y-12 border-2 border-slate-100 p-10 lg:p-16 rounded-xl relative">
              ...
              <div className="space-y-8 pt-8 text-lg leading-relaxed text-justify text-slate-800">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-y-2 pl-4 text-base">
                      <div className="flex border-b border-slate-100 py-1">
                        ...
                        <span className="font-bold text-slate-900">{userProfile?.fullName || user?.displayName || t("정보 없음")}</span>
                      </div>
                      <div className="flex border-b border-slate-100 py-1">
                        <span className="w-40 font-black text-slate-500">{t('외국인등록번호')}</span>
                        <span className="font-bold text-slate-900">{userProfile?.foreignerRegistrationNumber || t("정보 확인 중")}</span>
                      </div>
                      <div className="flex border-b border-slate-100 py-1">
                        <span className="w-40 font-black text-slate-500">{t('연락처')}</span>
                        <span className="font-bold text-slate-900">{userProfile?.phoneNumber || user?.phoneNumber || t("정보 없음")}</span>
                      </div>
                      <div className="flex border-b border-slate-100 py-1">
                        <span className="w-40 font-black text-slate-500">{t('환급 및 정산 계좌')}</span>
                        <span className="font-bold text-slate-900">
                          {currentApp?.bankName ? `${t(currentApp.bankName)} / ${currentApp.accountNumber || ''}` : t("미지정")}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="pt-20 text-center space-y-10">
                  <p className="text-2xl font-bold text-slate-900">{formatDocumentDate(currentApp?.applicationDate || new Date().toISOString())}</p>

                  <div className="flex flex-col items-center gap-4">
                    <p className="text-xl font-bold text-slate-900">{t('이용자 (서명)')} : {userProfile?.fullName || user?.displayName}</p>
