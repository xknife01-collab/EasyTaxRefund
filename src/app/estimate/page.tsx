      ...
      <main className={`flex-1 container mx-auto ${isSimulation ? 'px-2 py-2' : 'px-4 pt-20 pb-12 sm:py-8 lg:py-24'}`}>
        <div className={`max-w-2xl mx-auto ${isSimulation ? 'space-y-3' : 'space-y-6 sm:space-y-8'}`}>
          ...
          <div className="relative">
            ...
            {step === 5 && (
              <Step5AuthWaitingView
                onPrev={() => { setStep(4); saveProgress(4); }}
                authMethod={authMethod}
                authSession={authSession}
                language={language}
                preFilterEstimate={preFilterEstimate}
                setIsVipChatOpen={setIsVipChatOpen}
                ocrResult={ocrResult}
                setOcrResult={setOcrResult}
                isOcrLoading={isOcrLoading}
                handleCarrierOcrUpload={handleCarrierOcrUpload}
                applyOcrName={applyOcrName}
                handleFinalVerifyAndAnalyze={handleFinalVerifyAndAnalyze}
                loading={loading}
              />
            )}

            {step === 6 && (
              <Step6AnalysisLoadingView
