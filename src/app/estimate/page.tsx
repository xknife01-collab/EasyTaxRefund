...
import { DocumentViewerModal } from "@/components/estimate/modals/DocumentViewerModal";
import { TermsConsentModal } from "@/components/estimate/modals/TermsConsentModal";
...
export default function EstimatePage() {
  ...
  // 🚨 단계 변경 전역 이벤트 알림 및 온디맨드(버튼 클릭/질문 시) 가이드 이벤트 처리
  useEffect(() => {
    window.dispatchEvent(new CustomEvent("ktrs-step-change", { detail: { step } }));

    const handleStuckEvent = () => setIsStepGuideModalOpen(true);
    window.addEventListener("ktrs-user-stuck", handleStuckEvent);
    window.addEventListener("open-step-guide", handleStuckEvent);

    return () => {
      window.removeEventListener("ktrs-user-stuck", handleStuckEvent);
      window.removeEventListener("open-step-guide", handleStuckEvent);
    };
  }, [step]);
  ...
  return (
    ...
    >
      ...
      {!isSimulation && <Navbar />}
      <main className={`flex-1 container mx-auto ${isSimulation ? 'px-2 py-2' : 'px-4 pt-20 pb-12 sm:py-8 lg:py-24'}`}>
              ...
              <div className="relative animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <Card className="rounded-3xl border border-[#b88c30]/30 shadow-2xl overflow-hidden bg-[#0b192c]">
                  ...
                  <CardHeader className="text-center py-8 sm:py-12 bg-[#0b192c] text-white relative overflow-hidden">
                    ...
                    />
                    <div className="absolute top-0 right-0 p-10 opacity-5"><Banknote className="h-48 w-48 text-[#b88c30]" /></div>

                    {/* Official Badge Line - 로고 없이 텍스트만 */}
              ...
              <div className="relative animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <Card className="rounded-3xl border border-[#b88c30]/30 shadow-2xl overflow-hidden bg-[#0b192c]">
                  ...
                  <CardHeader className="text-center py-8 sm:py-10 bg-[#0b192c] text-white relative overflow-hidden">
                    ...
                    <p className="relative z-10 text-slate-400 font-bold text-sm mt-2">{t('신분증 정보를 확인하여 감면 대상을 판별합니다.')}</p>
                    <div className="relative z-10 h-px w-16 bg-[#b88c30] mx-auto mt-4" />
                  </CardHeader>
                  ...
                  <CardContent className="space-y-5 p-5 sm:p-8 bg-[#0d1e30]">
                    ...
                    <div className="p-5 bg-white/5 rounded-2xl border border-[#b88c30]/20 relative overflow-hidden">
                      ...
                      <div className="relative flex flex-col sm:flex-row items-center gap-5">
                        <div className="shrink-0 relative">
                          ...
                          <Image src="/certified_security_seal_premium_1774150786685.png" alt="Certified Security" width={72} height={72} className="relative" />
                        </div>
                        <div className="space-y-3 flex-1 text-center sm:text-left">
                          <div className="flex items-center justify-center sm:justify-start gap-2">
                            <h4 className="text-base font-black text-white">{t('security_card_title')}</h4>
                            <Badge className="bg-[#b88c30]/10 text-[#b88c30] border border-[#b88c30]/30 text-[9px] font-black">{t('security_certified')}</Badge>
                          </div>
                    ...
                    {!isCameraActive ? (
                      <div onClick={startCamera} className="border-2 border-dashed border-[#b88c30]/30 rounded-2xl p-10 text-center bg-[#b88c30]/5 cursor-pointer hover:bg-[#b88c30]/10 transition-all group">
                        ...
                        <p className="text-xs font-bold text-slate-500 mt-2">{t('되도록 외국인 등록증을 촬영 해주세요. 그래야 정확한 정보가 입력 됩니다.')}</p>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <div className="relative w-full aspect-video rounded-2xl overflow-hidden shadow-2xl bg-black">
                          ...
                          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div className="w-[85%] aspect-[1.58] border-4 border-dashed border-white/80 rounded-2xl shadow-[0_0_0_9999px_rgba(0,0,0,0.5)] flex flex-col items-center justify-center">
                              <span className="text-white text-xs font-black bg-black/60 px-3 py-1 rounded-full uppercase tracking-wider backdrop-blur-sm animate-pulse">{t('신분증을 점선에 맞춰주세요')}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex gap-4">
                          <Button type="button" onClick={() => { const stream = videoRef.current?.srcObject as MediaStream; stream?.getTracks().forEach(track => track.stop()); setIsCameraActive(false); }}
                            className="h-14 w-1/3 rounded-2xl font-bold bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10" disabled={loading}>
                            {t('취소')}
                          </Button>
                          <Button type="button" onClick={captureAndScan} className="flex-1 h-14 bg-[#b88c30] hover:bg-[#cfa54c] text-[#0b192c] text-base font-black rounded-2xl shadow-xl" disabled={loading}>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-start gap-3 p-4 bg-emerald-950/40 rounded-2xl border border-emerald-800/30">
                        ...
                        <p className="text-[12px] font-black text-emerald-400 leading-relaxed">{t('본 환급은 합법적 권리로, 비자(E-9, E-7, F-2 등) 연장이나 체류 자격에 어떠한 불이익도 없습니다.')}</p>
                      </div>
                      <div className="flex items-start gap-3 p-4 bg-white/5 rounded-2xl border border-white/10">
                        ...
                        <div>
                          <p className="text-[12px] font-black text-white">{t('입력하신 정보는 은행 수준(AES-256)으로 암호화됩니다.')}</p>
                      ...
                      <div className="grid gap-4">
                        <div className="space-y-2">
                          <div className="flex flex-col gap-1">
                            <Label className="text-xs font-black text-[#b88c30] uppercase tracking-widest">{t('영문 성명 (NAME)')}</Label>
                            <p className="text-[10px] text-amber-400 font-bold">{t('* 정확한 조회를 위해 성과 이름을 꼭 띄어서 입력해 주세요.')}</p>
                          </div>
                          <input
                            id="step2-name-input"
                            ...
                            onChange={(e) => {
                              ...
                              if (newName.length > 3) { prefetchNameOptimization(newName); }
                            }}
                        ...
                        <div className="space-y-2">
                          <Label className="text-xs font-black text-[#b88c30] uppercase tracking-widest">{t('외국인 등록번호')}</Label>
                          <input
                            id="step2-reg-input"
            ...
            {step === 6 && !analysisError && (
              <Card className="rounded-2xl sm:rounded-[3rem] border-none shadow-2xl py-12 sm:py-24 text-center bg-[#0b192c] text-white relative overflow-hidden">
                ...
                <div className="absolute inset-0 opacity-[0.03] pointer-events-none"
                  style={{ backgroundImage: "repeating-linear-gradient(45deg, #b88c30 0px, #b88c30 1px, transparent 1px, transparent 50%)", backgroundSize: "30px 30px" }}
                />
                <CardContent className="space-y-10 relative z-10">
                  ...
                  <div className="space-y-8">
                    <div className="space-y-3">
                      <h2 className="text-2xl sm:text-3xl font-black font-headline text-white tracking-tight">{t('데이터를 분석 중입니다.')}</h2>
            ...
            {step === 6 && analysisError && (
              <Card className="rounded-2xl sm:rounded-[2.5rem] border-none shadow-2xl overflow-hidden bg-[#0b192c] animate-in fade-in slide-in-from-bottom-6 duration-500">
                ...
                <CardHeader className="text-center py-8 sm:py-12 bg-[#0b192c] border-b border-[#b88c30]/20 relative overflow-hidden">
                  ...
                  <CardTitle className="text-2xl sm:text-3xl font-black text-white relative z-10">{t('AI 오류 진단 리포트')}</CardTitle>
                  ...
                  <div className="p-5 bg-white/5 rounded-2xl border border-[#b88c30]/20 space-y-3">
                    ...
                    {analysisError.code === "NAME_MISMATCH" && nameSuggestions.length > 0 && (
                      <div className="mt-5 pt-5 border-t border-[#b88c30]/15 space-y-3">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('다른 이름 조합으로 바로 시도하기')}</p>
      ...
      {/* 개인정보 및 CMS 출금 동의 모달 */}
      <TermsConsentModal
        piConsentOpen={piConsentOpen}
        setPiConsentOpen={setPiConsentOpen}
        cmsModalOpen1={cmsModalOpen1}
        setCmsModalOpen1={setCmsModalOpen1}
        cmsModalOpen2={cmsModalOpen2}
        setCmsModalOpen2={setCmsModalOpen2}
        cmsModalOpen3={cmsModalOpen3}
      ...
      <FindHometaxIdModal
        ...
        findIdStep={findIdStep}
        setFindIdStep={setFindIdStep}
        findIdForm={findIdForm}
        setFindIdForm={setFindIdForm}
        findIdLoading={findIdLoading}
        findIdResult={findIdResult}
        handleRequestFindIdSms={handleRequestFindIdSms}
        handleVerifyFindIdSms={handleVerifyFindIdSms}
        onSelectFoundId={(id) => {
          setHometaxId(id);
          setFindIdOpen(false);
          setAuthTab('login');
      ...
      <ResetHometaxPwModal
        ...
        resetPwStep={resetPwStep}
        setResetPwStep={setResetPwStep}
        resetPwForm={resetPwForm}
        setResetPwForm={setResetPwForm}
        resetPwLoading={resetPwLoading}
        handleRequestResetPwSms={handleRequestResetPwSms}
        handleVerifyResetPwSms={handleVerifyResetPwSms}
      />
      ...
      <ResumeApplicationModal
        isOpen={showResumeDialog && !isSimulation}
