      ...
      <main className="flex-1 flex flex-col items-center w-full">
        ...
        <section className="w-full bg-white border-b border-slate-100 py-8 px-6">
          <div className="max-w-5xl mx-auto flex flex-wrap items-center justify-center gap-10">
            ...
            <div className="flex flex-col items-center gap-2">
              <img src="/official_nts_carrier_badge_v2_1774141326494.png" alt={t("공식연동")} className="h-16 object-contain" />
              <span className="text-xs font-black text-slate-400 tracking-wide">{t("공인 세무사 직접 대행")}</span>
            </div>
            <div className="h-12 w-px bg-slate-200 hidden sm:block" />
            <div className="flex flex-col items-center gap-2">
              <img src="/certified_security_seal_premium_1774150786685.png" alt={t("보안인증")} className="h-16 object-contain" />
        ...
        <section className="w-full bg-[#0f1e36] py-20 px-6">
          <div className="max-w-5xl mx-auto text-center space-y-7">
            <div className="flex items-center justify-center gap-3">
              <div className="h-px w-10 bg-[#b88c30]" />
              <span className="text-[#b88c30] text-xs font-black tracking-widest uppercase">{t("가격 정책")}</span>
              <div className="h-px w-10 bg-[#b88c30]" />
            </div>
            <h2 className="text-3xl sm:text-5xl font-black text-white leading-tight break-keep">
              {t("선결제 0원, 오직 환급 성공 시에만")}<br />
              <span className="text-[#b88c30]">{t("후불제 자동 정산")}</span>
            </h2>
        ...
        <section className="w-full py-24 px-6 bg-white">
          <div className="max-w-5xl mx-auto space-y-16">
            <div className="space-y-4">
              ...
              <h2 className="text-3xl sm:text-5xl font-black text-[#0f1e36] leading-tight">{t("3가지 핵심 혜택")}</h2>
              <div className="h-1 w-12 bg-[#b88c30] rounded-full" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { icon: <Sparkles className="h-7 w-7" />, bg: "bg-[#b88c30]/10 text-[#b88c30]", accent: "border-t-[#b88c30]", tag: t("진행 리스크 0%"), tagColor: "text-[#b88c30]", title: t("초기 비용 0원, 완전 무료 시작"), desc: t("서비스 신청 시 필요한 비용은 전혀 없습니다. 예상 환급액 조회부터 전문 세무사의 전담 검토 단계까지, 신청 시점에 내야 할 돈은 단 1원도 없습니다.") },
                { icon: <RotateCcw className="h-7 w-7" />, bg: "bg-red-50 text-red-500", accent: "border-t-[#0f1e36]", tag: t("성공 보수 100%"), tagColor: "text-red-500", title: t("환급 거절/실패 시 수수료 0원"), desc: t("세무서 심사 결과 환급액이 나오지 않으면 서비스 수수료도 청구되지 않습니다. 고객님은 비용 손실이나 리스크가 0%이므로 안심하고 권리를 찾으세요.") },
                { icon: <Coins className="h-7 w-7" />, bg: "bg-emerald-50 text-emerald-600", accent: "border-t-[#b88c30]", tag: t("후불 자동 정산"), tagColor: "text-emerald-600", title: t("환급금 입금된 후 후불 정산"), desc: t("국세청에서 고객님 명의의 은행 계좌로 환급금을 직접 입금해 드리면, 그 이후에 후불 정산합니다.") },
              ].map((c, i) => (
        ...
        <section className="w-full bg-slate-50 border-t border-slate-100 py-24 px-6">
          <div className="max-w-5xl mx-auto space-y-16">
            <div className="space-y-4">
              ...
              <h2 className="text-3xl sm:text-5xl font-black text-[#0f1e36] leading-tight break-keep">{t("실제 환급액 계산 예시")}</h2>
              <div className="h-1 w-12 bg-[#b88c30] rounded-full" />
              <p className="text-slate-500 font-bold text-base">
                {t("월급 250만원 / 중소기업 재직")}{" "}
                <span className="text-[#b88c30] font-black">{t("3년")}</span>{" "}
                {t("기준 / E-9 비자")}
              </p>
            </div>
            ...
            <p className="text-xs text-slate-400 font-bold">{t("* 위 금액은 예시이며 실제 환급액은 근무 기간, 소득, 비자 종류 등에 따라 다를 수 있습니다.")}</p>
          </div>
        </section>
          ...
          <div className="max-w-5xl mx-auto space-y-16">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
              <div className="lg:col-span-3 bg-white border border-t-4 border-t-[#b88c30] border-x-slate-200 border-b-slate-200 rounded-3xl p-8 sm:p-10 space-y-6 shadow-sm">
                {[
                  { icon: <CheckCircle2 className="h-6 w-6 text-[#b88c30] shrink-0 mt-1" />, title: t("국가공인 제휴 세무사의 전 과정 책임 대행"), desc: t("환급 신청서 작성부터 환급금이 통장에 입금되는 마지막 순간까지, 대한민국 국가공인 전문 세무사가 모든 심사와 행정 과정을 전담하여 책임 처리합니다.") },
                  { icon: <CheckCircle2 className="h-6 w-6 text-[#b88c30] shrink-0 mt-1" />, title: t("제휴 세무사의 1:1 맞춤 정밀 검토"), desc: t("단순 계산 오류나 비자 타입(E-9, E-7 등) 누락을 최소화하여 환급 성공률을 극대화합니다.") },
                  { icon: <UserCheck className="h-6 w-6 text-[#b88c30] shrink-0 mt-1" />, title: t("비대면 자동화 세무 서류 생성 솔루션"), desc: t("지난 5년 치의 소득세 납부 데이터를 안전하게 암호화하여 처리하는 독자적인 기술 솔루션을 제공합니다.") },
                  { icon: <ShieldCheck className="h-6 w-6 text-[#b88c30] shrink-0 mt-1" />, title: t("국세청 공식 절차(경정청구) 전담 마크"), desc: t("접수부터 국세청 심사관 대응까지 환급금이 통장에 최종 입금되는 순간까지 모든 프로세스를 밀착 케어합니다.") },
                ].map((item, i) => (
                  <div key={i} className="flex gap-4">
                    {item.icon}
                    <div>
                      <h4 className="font-black text-[#0f1e36] text-base mb-1">{t(item.title)}</h4>
                      <p className="text-slate-500 font-bold text-sm leading-relaxed break-keep">{t(item.desc)}</p>
                    </div>
                  </div>
                ))}
              </div>
              <div className="lg:col-span-2 bg-[#0f1e36] rounded-3xl p-8 sm:p-10 flex flex-col justify-between shadow-2xl shadow-slate-900/20">
                <div className="space-y-5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[#b88c30] text-xs font-black uppercase tracking-widest">{t("준수 사항 (COMPLIANCE)")}</span>
                  </div>
                  <div className="h-px w-10 bg-[#b88c30]" />
                  <h3 className="text-2xl font-black text-white leading-tight">{t("합법적인 세무 프로세스")}</h3>
                  <p className="text-slate-400 font-bold text-sm leading-relaxed break-keep">{t("* 본 서비스는 대한민국 국가공인 전문 세무법인과의 공식 기술 제휴를 통해 운영되며, 세무 분석 및 신청 지원 솔루션을 제공합니다. 고객님의 실제 세무 신고 및 환급 대행 업무는 제휴 세무법인의 책임 하에 안전하고 적법하게 처리되므로 안심하셔도 됩니다.")}</p>
                </div>
                <div className="pt-8">
                  <Link href="/estimate" className="w-full h-14 bg-[#b88c30] hover:bg-[#b88c30]/90 text-[#0f1e36] font-black rounded-2xl flex items-center justify-center gap-2 transition-all hover:scale-[1.02] shadow-xl text-base">
                    {t("지금 무료로 조회하기")} <ArrowRight className="h-5 w-5" />
                  </Link>
                </div>
              </div>
            </div>
