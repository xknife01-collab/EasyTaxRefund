...
// [EasyTax SEO] 대학 유학생 3.3% 환급
// Route: /student-refund/[slug]
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  return {
    title: `[${title}] D-2 Student Part-Time 3.3% Tax Refund | EasyTax Korea`,
...
export default async function EasyTaxStudentRefundPage({ params, searchParams }: Props) {
  const { slug } = await params;
  const sp = searchParams ? await searchParams : {};
  const lang = sp?.lang || 'en';
  const langInfo = LANG_MAP[lang] || LANG_MAP['en'];
  const title = slugToTitle(slug);
  const cta = `/welcome?lang=${lang}&utm_source=google_seo&utm_medium=organic&utm_campaign=student-refund_${slug}`;
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: '#0a0f1e', minHeight: '100vh', color: '#f8fafc' }}>
      ...
      <section style={{ maxWidth: '900px', margin: '0 auto', padding: '60px 24px 40px', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: '30px', fontWeight: 900, lineHeight: 1.3, marginBottom: '16px' }}>
          {title} — D-2 Student Part-Time 3.3% Tax Refund
        </h1>
